...
#PERCENTAGES ARE WRONG SOMEHOW??? CHECK ROWS??
#find better names to sort & calculate all other conditions
...
my_data <- my_data %>% 
  mutate(myopic = case_when(MYOPIC == "Yes" ~ 1, 
                            MYOPIC == "No" ~ 0))

my_data <- my_data %>% 
  mutate(parents_fct = as.factor(parents_int), myopic = as.factor(myopic))
glimpse(my_data)
