...
#ROC CURVE 
roc_rf <- roc(myopia_balanced$MYOPIC, as.numeric(oob_predictions))
roc_xgb <- roc(testLabel, pred)
roc_svm <- roc(test_data$MYOPIC, probabilities[, 2])

# Color-blind-friendly colors
color_rf <- "#0072B2"   # Dark Blue
color_xgb <- "#D55E00"  # Orange
color_svm <- "#009E73"  # Dark Green

# Assuming you have these ROC curves and AUC values
auc_rf <- auc(roc_rf)
auc_xgb <- auc(roc_xgb)
auc_svm <- auc(roc_svm)

plot(roc_rf, col = color_rf, main = "ROC Curves for Random Forest, XGBoost, and SVM", legacy.axes = TRUE)
...
legend("bottomright", legend = legend_text,
       col = c(color_rf, color_xgb, color_svm), lty = 1, cex = 0.8)
