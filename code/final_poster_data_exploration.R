...
# For data analysis like t-test and logistic regression
...
# Violin Plot for SPHEQ VS MYOPIC
ggplot(myopiaGraph,
       aes(x = MYOPIC,
           y = SPHEQ,
           fill = MYOPIC))  +
  ...
  scale_fill_manual(values = c("Yes" = "darkorange", "No" = "dodgerblue")) +
  theme_minimal() +
  labs(title = "Spherical Equivalent of Refractive Error and Myopia",
       x = "Myopic",
       y = "SPHEQ")
...
#Finding SPHEQ median
median_SPHEQ <- median(myopiaData$SPHEQ)
print(paste("Median SPHEQ: ", median_SPHEQ))

# Calculate Medians for Non-Myope (0) and Myope (1)
medians <- myopiaData %>%
  group_by(MYOPIC) %>%
  summarize(median_SPHEQ = median(SPHEQ))

print(medians)

# Finding SPHEQ median
median_SPHEQ <- median(myopiaData$SPHEQ)
print(paste("Overall Median SPHEQ: ", median_SPHEQ))

# Finding SPHEQ median for non-myope (0) and myope (1)
medians_by_group <- myopiaData %>%
  group_by(MYOPIC) %>%
  summarize(median_SPHEQ = median(SPHEQ))
...
ggplot(myopiaGraph,
  ...
  scale_fill_manual(values = c("Yes" = "darkorange", "No" = "dodgerblue")) +
  theme_minimal() +
  labs(title = "Anterior Chamber Depth and Myopia",
       x = "Myopic",
       y = "Anterior Chamber Depth")
...
ggplot(myopiaGraph,
  ...
  scale_fill_manual(values = c("Yes" = "darkorange", "No" = "dodgerblue")) +
  theme_minimal() +
  labs(title = "Hours on Sports vs. Myopia",
       x = "Myopic",
       y = "Hours on Sports")

sportResult <- t.test(myopiaData$SPORTHR ~ myopiaData$MYOPIC)
print(sportResult)

#Logistic model for sports hour vs. myopia
set.seed(0)
sport_split <- initial_split(myopiaData, prop = 0.7) 
train_sport <- training(sport_split)
test_sport <- testing(sport_split)

sport_model <- glm(MYOPIC~ SPORTHR, family=binomial, data=train_sport)
summary(sport_model)

sport_model %>%
  tbl_regression(estimate_fun = function(x) style_number(x, digits = 3), exponentiate = TRUE)

sport_prob <- sport_model %>% 
  predict(test_sport,type="response")

# Calculates model's accuracy in prediction
predicted.classes <- ifelse(sport_prob > 0.5, "1", "0")
mean(predicted.classes == test_sport$MYOPIC)
# This has a 88% accuracy rate
...
medians <- myopiaData %>%
  group_by(MYOPIC) %>%
  summarize(median_SPORTHR = median(SPORTHR))

print(medians)

# Finding SPHEQ median
median_SPORTHR <- median(myopiaData$SPORTHR)
print(paste("Overall Median SPHEQ: ", median_SPORTHR))
...
print(medians_by_group)
#-------------------------------------------------------------------------------
# DADMY & MOMMY vs. MYOPIC

myopiaParents <- read.csv("myopia_og.csv", head = TRUE, sep = ";")

myopiaParents <- myopiaParents %>% 
   mutate(MOMMY = case_when(MOMMY == 1 ~ "Myopic Mom", 
                            MOMMY == 0 ~ "Non-Myopic Mom"))
myopiaParents <- myopiaParents %>% 
  mutate(DADMY = case_when(DADMY == 1 ~ "Myopic Dad", 
                           DADMY == 0 ~ "Non-Myopic Dad"))
myopiaParents <- myopiaParents %>% 
  mutate(MYOPIC = case_when(MYOPIC == 1 ~ "Yes", 
                           MYOPIC == 0 ~ "No"))

myopiaParents <- myopiaParents %>%
  mutate(MOMMY = as.factor(MOMMY),
         DADMY = as.factor(DADMY), 
         MYOPIC = as.factor(MYOPIC))

myopiaParents <- myopiaParents %>% 
  filter(AGE !=9)

glimpse(myopiaParents)

#THISSSS
ggplot(myopiaParents, aes(x = DADMY, fill = MYOPIC)) +
  geom_bar(position = "dodge") +
  facet_grid(. ~ MOMMY) +
  scale_fill_manual(values = c("Yes" = "darkorange", "No" = "dodgerblue")) +
  theme_minimal() +
  labs(title = "Myopia by Mom and Dad's Myopic Status",
       x = "Parent Myopic Status",
       y = "Count",
       fill = "Myopic")

# IGNORE THIS
ggplot(myopiaGraph, aes(x = PARENTS_MYOPIC, fill = MYOPIC)) +
  geom_bar(position = "dodge") +
  theme_minimal() +
  labs(title = "Myopia Status by Parents' Myopia Status",
       x = "Parents' Myopia Status",
       y = "Count",
       fill = "Myopic") +
  scale_x_discrete(labels = c("Mom has myopia & Dad does not",
                              "Dad has myopia & Mom does not",
                              "Both do not have myopia",
                              "Both do have myopia")) +
