...
#just mutating/recategorizing variables
...
#basic bar graphs examining mom's and dad's myopic status respectively
...
#bar graph displaying both parents' myopic status in relation to their child's
#ran chi-squared test - SMOL p-value
...
#percentages of each subset in the sample poulation
...
neither_df <- par_df %>% 
  filter(PARENTS == conditionNM)
filter_neither <- nrow(neither_df)
one_df <- par_df %>% 
  filter(PARENTS == conditionOM)
filter_one <- nrow(one_df)
both_df <- par_df %>% 
  filter(PARENTS == conditionBM) 
filter_both <- nrow(both_df)
...
#23.79% non-myopic with neither myopic parents in entire population
...
#43.69% non-myopic with one myopic parent in entire population
...
#19.42% non-myopic with both myopic parent in entire population
...
#0.81% myopic with neither myopic parents in entire population
...
#6.63% myopic with one myopic parent in entire population
...
#5.66% myopic with both myopic parents in entire population 

``` 

```{r}

#logistic regression model - parents' myopic status (response variable), participants' myopic status (explanatory variable)
